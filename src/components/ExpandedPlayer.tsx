  ...
  useEffect(() => {
    ...
    const startDrawing = () => {
      const draw = () => {
        ...
        // 2. Draw Analyser (Native iOS vDSP) - AQUI ESTA LA FORMA ORIGINAL DE DIBUJO
        if (ctx && canvas && (canvas as any).nativeFftData) {
          ...
          for (let i = 0; i < bufferLength; i++) {
            ...
            ctx.beginPath();
            if (ctx.roundRect) {
              // DIBUJA DESDE ABAJO HACIA ARRIBA: `canvas.height - barHeight`
            ...
            // AVANZA HACIA LA DERECHA (No simetrico)
            x += barWidth;
          }
